...
var dayText = ["壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "拾", "拾壹", "拾贰", "拾叁", "拾肆", "拾伍", "拾陆", "拾柒", "拾捌", "拾玖", "贰拾", "贰拾壹", "贰拾贰", "贰拾叁", "贰拾肆", "贰拾伍", "贰拾陆", "贰拾柒", "贰拾捌", "贰拾玖", "叁拾", "叁拾壹"];
var twoHourText = ["子时", "丑时", "寅时", "卯时", "辰时", "巳时", "午时", "未时", "申时", "酉时", "戌时", "亥时"];
var weekText = ["星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"];
// var hourText = ["零点", "一点", "两点", "三点", "四点", "五点", "六点", "七点", "八点", "九点", "十点", "十一点", "十二点", "十三点", "十四点", "十五点", "十六点", "十七点", "十八点", "十九点", "二十点", "二十一点", "二十二点", "二十三点"];
var hourText = ["零","壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "拾", "拾壹", "拾贰", "拾叁", "拾肆", "拾伍", "拾陆", "拾柒", "拾捌", "拾玖", "贰拾", "贰拾壹", "贰拾贰", "贰拾叁"];
...
var minuteText = ["壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "拾", "拾壹", "拾贰", "拾叁", "拾肆", "拾伍", "拾陆", "拾柒", "拾捌", "拾玖", "贰拾", "贰拾壹", "贰拾贰", "贰拾叁", "贰拾肆", "贰拾伍", "贰拾陆", "贰拾柒", "贰拾捌", "贰拾玖", "叁拾", "叁拾壹"
,"叁拾壹","叁拾贰","叁拾叁","叁拾肆","叁拾伍","叁拾陆","叁拾柒","叁拾捌","叁拾玖","肆拾","肆拾壹","肆拾贰","肆拾叁","肆拾肆","肆拾伍","肆拾陆","肆拾柒","肆拾捌","肆拾玖","伍拾","伍拾壹","伍拾贰","伍拾叁","伍拾肆","伍拾伍","伍拾陆","伍拾柒","伍拾捌","伍拾玖","陆拾"];
...
var secondsText =["壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖", "拾", "拾壹", "拾贰", "拾叁", "拾肆", "拾伍", "拾陆", "拾柒", "拾捌", "拾玖", "贰拾", "贰拾壹", "贰拾贰", "贰拾叁", "贰拾肆", "贰拾伍", "贰拾陆", "贰拾柒", "贰拾捌", "贰拾玖", "叁拾", "叁拾壹"
    ,"叁拾壹","叁拾贰","叁拾叁","叁拾肆","叁拾伍","叁拾陆","叁拾柒","叁拾捌","叁拾玖","肆拾","肆拾壹","肆拾贰","肆拾叁","肆拾肆","肆拾伍","肆拾陆","肆拾柒","肆拾捌","肆拾玖","伍拾","伍拾壹","伍拾贰","伍拾叁","伍拾肆","伍拾伍","伍拾陆","伍拾柒","伍拾捌","伍拾玖","陆拾"];
...
function runtime() {
    ...
    var timeArr = [0, month, day-1, week, hour, hour, minute, seconds]
...
function addColor(timeArr) {
    ...
    textList[0][1][0].classList.add('now');
    textList[0][1][0].onclick=function () {
        show();
    };
    for (var i = 1; i < timeArr.length; i++) {
        var index = timeArr[i];
        if(i==4){
            textList[i][1][Math.ceil(index/2)].classList.add('now')
        }else{
            var index = timeArr[i];
...
function changeImg() {
    ...
    document.getElementById("groundImg").removeAttribute('src');
    document.getElementById("groundImg").setAttribute('src',img);
    document.getElementById("groundImg").setAttribute('background-size', cover);
    // document.getElementById("groundImg").setAttribute('height', 1400);
}
// (function () {
//     var imgDiv = document.getElementById("img")
//     // var obj =document.createElement("<img src=\""+ img +"\" height=\\\"100%\\\"/>"); //添加哪类节点
//     var imgObj =document.createElement("img"); //添加哪类节点
//     imgObj.setAttribute('src',img);
//     imgObj.setAttribute('id',"groundImg");
//     imgObj.setAttribute('height', 1400);
...
function rotateTransition(timeArr) {
    ...
    for (var i in textList) {
        for (var j in textList[i][1]) {
            var flg = timeArr[i];
            if(i==4){
                flg = Math.ceil(timeArr[i]/2)
            }
            var temp = textList[i][1][j];
            var deg = 360 / textList[i][0].length * (j - flg);
